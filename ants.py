...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        return None
        #self.place.bees - self.place  # REPLACE THIS LINE
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        bee_length = len(self.place.bees)
        for i in range(len(self.place.bees)):
            x = bee_length - len(self.place.bees)
            self.place.bees[i - x].reduce_health(amount)
        new_bee_length = len(self.place.bees)
        if amount >= self.health:
            for i in range(len(self.place.bees)):
                x = new_bee_length - len(self.place.bees)
                self.place.bees[i - x].reduce_health(self.damage)
...
# END Problem 12
    ...
    def action(self, gamestate):
        ...
        super().action(gamestate)
        location = self.place
        while location.exit != None:
            if location.exit.ant != None:
                if location.exit.ant.is_container:
                    if location.exit.ant.ant_contained != None:
                        location.exit.ant.ant_contained.buff()        
                location.exit.ant.buff()
...
#         d = {}
#         while location != Hive:
#             if location.insect != None:
#                 for i in location.insect:
#                     d[i.insect] = counter
